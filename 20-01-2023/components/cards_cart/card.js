import { cE, qS, globalFetch } from "../../utils.js"
...
const exportCard = () => {
    ...
    const selectorEl = qS("#cat__selector");
    ...
    // GENERATORE DI CARD IN BASE ALLA CATEGORIA
    globalFetch("categories", "GET")
        .then((data) => {
            filterSelector(data);
        });
    ...
    selectorEl.addEventListener('change', (event) => {
        globalFetch(event.target.value === "tutti" ? "products" : `categories/${event.target.value}/products`, 'GET')
            .then(data => cardCreateEl(data))
            .catch(e => {
                console.log(`Errore nel caricamento dell'API: ${e}`)
                cardContainerEl.textContent = "C'è stato un errore, prova ricaricando la pagina..."
